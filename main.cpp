...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (vida <= 0) {
            ...
            DrawText(TextFormat("Puntaje: %i", puntaje), GetScreenWidth() / 2 - 70, GetScreenHeight() / 2 + 10, 30, RAYWHITE);

            // Mejor puntaje
            if (puntaje > mejorPuntaje) {
                mejorPuntaje = puntaje;

                DrawText("NEW HIGH SCORE!", GetScreenWidth() / 2 - 150, GetScreenHeight() / 2 + 40, 30, GOLD);
            ...
            DrawText("Presiona ENTER para continuar...", GetScreenWidth() / 2 - 180, GetScreenHeight() / 2 + 80, 20, LIGHTGRAY);
            WaitTime(0.2f); // Pequeña espera para evitar múltiples detecciones
